  ...
  <div class="panel panel-default cd" id="filter_tipo_de_cuentas">
    <div class="panel-heading">
      <p class="panel-title roboto white">
        {{ title }}
        <span class="span-info-right" v-if="selected_by_tipo_cuentas !== 0"> ({{ selected_by_tipo_cuentas | numeral('0,0') }} empresas seleccionadas)</span>
      </p>
    </div>
    <div class="panel-body">
                          ...
                          <div class="panel panel-default cd">
                            <div class="panel-heading">
                              <p class="panel-title roboto white">
                                ...
                                <span class="span-info-right" v-if="tipo_cuentas.length !== 0"> {{ tipo_cuentas.length }}</span>
                              </p>
                            </div>
                          </div>
                        </div>
      ...
      <div v-if="search.tipo_cuentas && search.tipo_cuentas.length === 0 && !loading" class="alert alert-dismissible alert-primary">
  ...
  export default {
    ...
    watch: {
      tipo_cuentas: function (newProvincesLocalidad) {
        this.selected_by_tipo_cuentas = this.numberCompaniesSelected((this.isAllProvincesLocalidad(newProvincesLocalidad))? this.search.tipo_cuentas : newProvincesLocalidad)
    ...
    methods: {
      ...
      numberCompaniesSelected(newSelectedCompanies) {
        ...
        if (Array.isArray(newSelectedCompanies)) {
          newSelectedCompanies.forEach((item) => {
            let result = inArrayObjectTreeselect(this.search.tipo_cuentas, item.id)
      ...
      handleChangeList (elemet, event){
        ...
        this.reapply = (this.areApplied)? true: this.areApplied
        let checkboxs = document.querySelectorAll('#selected_tipo_cuentas input[type="checkbox"]')
