      ...
      <form v-on:submit.prevent="validateRazonSocial" class="m-b-10">
        <div class="input-group">
          <input
            ...
            id="social_reasons1"
            class="form-control"
            placeholder="escribe aqui el nombre o razón social de la empresa"
            title="escribe aqui el nombre o razón social de la empresa"
          />
...
export default {
  ...
  methods: {
          ...
          .then(response => {
            if (
              ...
              this.social_reasons.empresas = removeDuplicates(
                this.social_reasons.empresas,
                "RazonSocial"
              );
            } else {
              this.social_reasons.empresas = response.empresas
                ? response.empresas
                : [];
            }
    ...
    apply() {
      if (
        this.selected_social_reasons &&
        this.selected_social_reasons.length !== 0
      ) {
        ...
        this.$store
          ...
          .then(response => {
            ...
            this.dataFrm = "";
            this.selected_social_reasons_string = JSON.stringify(
              this.selected_social_reasons
            );
            this.social_reasons = { total: 0, cantidad: 0, empresas: [] };
    ...
    editSearch() {
      ...
      setTimeout(() => {
        document.getElementById("social_reasons").focus();
      }, 100);
    },
    addSearch() {
      this.search_edit = true;
